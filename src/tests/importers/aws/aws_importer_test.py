...
def test_main_no_template_in_db(importer_lib, mocker):
    """AWS_IMPORT_TEMPLATE_ID set but no matching template row -> auto-run is
    disabled and polling still starts (the importer's primary job -- file
    ingestion -- doesn't depend on the optional workflow auto-run)."""
    import importers.aws.importer as aws_importer
    ...
    assert aws_importer.AWS_IMPORT_TEMPLATE_ID is None
