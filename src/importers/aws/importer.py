...
def _validate_startup_config() -> bool:
    ...
    A missing/invalid AWS_IMPORT_TEMPLATE_ID disables workflow auto-run
    rather than aborting startup — the importer's primary job is ingesting
    files, and auto-run is an optional add-on. ROBOT_ACCOUNT_USER_ID and
    AWS_SQS_QUEUE_URL remain hard requirements since nothing else works
    without them.
    ...
    with database.SessionLocal() as db:
        ...
        if (
            ...
            logger.warning(
                ...
                "in the database; disabling workflow auto-run.",
                AWS_IMPORT_TEMPLATE_ID,
            )
            AWS_IMPORT_TEMPLATE_ID = None
    return True
